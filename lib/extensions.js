/*!
 * node-rdkafka: lib/extensions.js
 */
...
/**
 * Get a human readable description of where node-rdkafka is running to support
 ...
 *
 * @return {Array} The currently installed node-rdkafka bindings
 *
 * @api public
 */
 ...
 * @return {Boolean} True, if node-rdkafka supports the current platform, false otherwise
 *
 * @api public
 */
...
/**
 * Get binary name.
 * If environment variable RDKAFKA_BINARY_NAME,
 * .npmrc variable rdkafka_binary_name or
...
function getBinaryName() {
  ...
  if (getArgument('--rdkafka-binary-name')) {
    binaryName = getArgument('--rdkafka-binary-name');
  } else if (process.env.RDKAFKA_BINARY_NAME) {
    binaryName = process.env.RDKAFKA_BINARY_NAME;
  } else if (process.env.npm_config_rdkafka_binary_name) {
    binaryName = process.env.npm_config_rdkafka_binary_name;
  } else if (pkg.nodeRdkafkaConfig && pkg.nodeRdkafkaConfig.binaryName) {
    binaryName = pkg.nodeRdkafkaConfig.binaryName;
  } else {
  ...
  return [binaryName, 'node-librdkafka.node'].join('_');
}

/**
 * Determine the URL to fetch binary file from.
 * By default fetch from the node-rdkafka distribution
 * site on GitHub.
 *
 * The default URL can be overridden using
 * the environment variable RDKAFKA_BINARY_SITE,
 * .npmrc variable rdkafka_binary_site or
 * or a command line option --rdkafka-binary-site:
 *
 *   node scripts/install.js --rdkafka-binary-site http://example.com/
 *
...
function getBinaryUrl() {
  var site = getArgument('--rdkafka-binary-site') ||
             process.env.RDKAFKA_BINARY_SITE  ||
             process.env.npm_config_rdkafka_binary_site ||
             (pkg.nodeRdkafkaConfig && pkg.nodeRdkafkaConfig.binarySite) ||
             'https://github.com/rdkafka/node-rdkafka/releases/download';
...
/**
 * Get binary dir.
 * If environment variable RDKAFKA_BINARY_DIR,
 * .npmrc variable rdkafka_binary_dir or
 * process argument --rdkafka-binary-dir is provided,
...
function getBinaryDir() {
  ...
  if (getArgument('--rdkafka-binary-dir')) {
    binaryDir = getArgument('--rdkafka-binary-dir');
  } else if (process.env.RDKAFKA_BINARY_DIR) {
    binaryDir = process.env.RDKAFKA_BINARY_DIR;
  } else if (process.env.npm_config_rdkafka_binary_dir) {
    binaryDir = process.env.npm_config_rdkafka_binary_dir;
  } else if (pkg.nodeRdkafkaConfig && pkg.nodeRdkafkaConfig.binaryDir) {
    binaryDir = pkg.nodeRdkafkaConfig.binaryDir;
  } else {
...
/**
 * Get binary path.
 * If environment variable RDKAFKA_BINARY_PATH,
 * .npmrc variable rdkafka_binary_path or
 * process argument --rdkafka-binary-path is provided,
 * select it by appending binary name, otherwise
...
function getBinaryPath() {
  ...
  if (getArgument('--rdkafka-binary-path')) {
    binaryPath = getArgument('--rdkafka-binary-path');
  } else if (process.env.RDKAFKA_BINARY_PATH) {
    binaryPath = process.env.RDKAFKA_BINARY_PATH;
  } else if (process.env.npm_config_rdkafka_binary_path) {
    binaryPath = process.env.npm_config_rdkafka_binary_path;
  } else if (pkg.nodeRdkafkaConfig && pkg.nodeRdkafkaConfig.binaryPath) {
    binaryPath = pkg.nodeRdkafkaConfig.binaryPath;
  } else {
...
function getCachePathCandidates() {
  return [
    process.env.npm_config_rdkafka_binary_cache,
...
/**
 * Get Rdkafka version information
 *
 * @api public
 */
...
function getVersionInfo(binding) {
  return [
    ['node-rdkafka', pkg.version, '(Wrapper)', '[JavaScript]'].join('\t'),
    ['librdkafka  ', binding.librdkafkaVersion(), '(Rdkafka Compiler)', '[C/C++]'].join('\t'),
  ].join(eol);
}
