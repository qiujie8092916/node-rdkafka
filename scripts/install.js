...
function checkAndDownloadBinary() {
  if (process.env.SKIP_RDKAFKA_BINARY_DOWNLOAD_FOR_CI) {
    console.log('Skipping downloading binaries on CI builds');
    return;
  }

  var cachedBinary = rdkafka.getCachedBinary(),
    cachePath = rdkafka.getBinaryCachePath(),
    binaryPath = rdkafka.getBinaryPath();

  if (rdkafka.hasBinary(binaryPath)) {
    console.log('node-rdkafka build', 'Binary found at', binaryPath);
    return;
  }
  ...
  download(rdkafka.getBinaryUrl(), binaryPath, function(err) {
    if (err) {
      console.error(err);
      return;
    }
    ...
    cachedBinary = path.join(cachePath, rdkafka.getBinaryName());
